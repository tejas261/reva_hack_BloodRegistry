...
contract BloodDonor{
    ...
    function donor_details_from_aadhar(string memory d_id) public view returns(string memory){
        uint256 index = index_to_donor[d_id];
        Donor memory b = donors[index] ;
        return b.d_blood_type ;
    }
...
contract BloodSample {
    enum BLOOD_STATUS{
            COLLECTED,
            TESTED_OK,
            TESTED_INFLAMMATORY_DISEASES, 
            TESTED_CHRONIC_DISEASES,
            EXPIRED,
            REQUESTED,
            SENT_TO_HOSPITAL,
    ...
    function collected(string memory d_id,string memory bb_id,string memory blood_id, string memory blood_type, string memory date_collected) public{
        bloodBags.push(blood({d_id: d_id, bb_id: bb_id, blood_id: blood_id, blood_type: blood_type, state: BLOOD_STATUS.COLLECTED , date_collected:date_collected}));
    ...
    function tested(string memory blood_id, string memory test) public  {
        ...
        string memory a = "OK";
        string memory c = "NOT OK PERMANENT";
        string memory d = "NOT OK TEMP";
        if (keccak256(abi.encodePacked(test)) == keccak256(abi.encodePacked(a))){
            b.state = BLOOD_STATUS.TESTED_OK;
        }else if(keccak256(abi.encodePacked(test)) == keccak256(abi.encodePacked(c))){
            BloodDonor d = new BloodDonor();
            d.block_donor(b.d_id);
            b.state = BLOOD_STATUS.TESTED_CHRONIC_DISEASES;
        }else if(keccak256(abi.encodePacked(test)) == keccak256(abi.encodePacked(d))){
            b.state = BLOOD_STATUS.TESTED_INFLAMMATORY_DISEASES;
        }
    }
